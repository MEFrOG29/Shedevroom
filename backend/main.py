...
def create_booking(
    ...
    if booking_data.time_slot.minute not in [0, 30] or booking_data.time_slot.second != 0:
        raise HTTPException(
            status_code=400,
    ...
    new_start = datetime.combine(booking_data.booking_date, booking_data.time_slot)
    ...
    clean_time_slot = booking_data.time_slot.replace(second=0, microsecond=0)
...
def get_available_slots(booking_date: date, db: Session = Depends(get_db)):
    ...
    for slot in all_slots:
        ...
        for b in bookings:
            is_pending_and_fresh = (b.status == "pending" and (now - b.created_at).total_seconds() < 1800)
            is_confirmed = (b.status == "confirmed")
