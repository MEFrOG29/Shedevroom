...
import * as styles from "../styles/bookingStyles";
    ...
    return (
        <div className={styles.calendarContainer}>
            <div className={styles.calendarHeader}>
                <button type="button" onClick={() => handleMonthChange('prev')} className={styles.calendarArrow}>&lt;</button>
                <span className={styles.calendarMonthTitle}>
                    {monthsNames[currentMonth]} {currentYear}
                </span>
                <button type="button" onClick={() => handleMonthChange('next')} className={styles.calendarArrow}>&gt;</button>
            </div>

            <div className={styles.weekDaysGrid}>
                {weekDays.map((day, idx) => (
                    <div key={idx} className={styles.weekDayName}>{day}</div>
                ))}
            </div>
            ...
            <div className={styles.daysGrid}>
                {calendarCells.map((cell, idx) => {
                    ...
                    if (isSelected) {
                        ...
                        cellClass = "w-[40px] h-[40px] flex items-center justify-center text-white/20 cursor-not-allowed opacity-40 bg-transparent";
                    } else if (isNeighborMonth) {
                        cellClass = `${styles.cellDayFree} opacity-50 hover:opacity-100`;
                    }
                    ...
                    return (
                        ...
                        >
                            <span className={styles.dayNumberText}>{cell.day}</span>
                        </div>
                    );
                })}
            </div>
